        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:gap-6">
          ...
          <div className="grid min-w-0 flex-1 grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {problems.map((item) => (
              <div
                key={item.label}
                className="flex flex-col items-center rounded-2xl bg-white px-3 py-6 text-center shadow-[0_10px_30px_rgba(80,30,50,0.06)]"
              >
                <Image
                  src={item.src}
                  alt={item.label}
